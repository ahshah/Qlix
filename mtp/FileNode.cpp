...
FileNode::FileNode(const QFileInfo& in_file, uint32_t in_parentID)
{
    _parentId = in_parentID;
    _name = in_file.fileName();
...
FileNode::FileNode(LIBMTP_file_t* in_head, count_t in_height)
{
        _itemId = in_head->item_id;
        _parentId = in_head->parent_id;
        _name = QString::fromUtf8(in_head->filename);
        _fileSize = in_head->filesize;
        _type = typeToString(in_head->filetype);
        _height = in_height;
        setupDefaultMetaData();
}


FileNode::FileNode(LIBMTP_track_t* in_head, count_t in_height)
{
        _itemId = in_head->item_id;
        _parentId = in_head->parent_id;
        _name = QString::fromUtf8(in_head->filename);
...
QString FileNode::GetFileName() const
{
    return _name;
}
...
QString FileNode::GetType() const
{
    return _type;
}

LIBMTP_filetype_t FileNode::GetMtpType()
{
    return stringToType();
}

LIBMTP_filetype_t FileNode::GetMtpType(const QString& in_suffix)
{
    QString temp = in_suffix.toUpper();
    return stringToType(temp);
}

QString FileNode::GetMtpType(LIBMTP_filetype_t in)
{
    return typeToString(in);
}


uint32_t FileNode::GetParentID() const //need to rename this
{
    return _parentId;
}
...
uint32_t FileNode::GetID() const
{
    return _itemId;
}

//debug
void FileNode::Sanity (LIBMTP_filetype_t in_type) const
{
    qDebug() << "Received this type: " << typeToString(in_type) << endl;
}

count_t FileNode::GetHeight() const
{
    return _height;
}

void FileNode::SetHeight(count_t in_height)
{
    _height = in_height;
    return;
}
...
//private:
void FileNode::setupDefaultMetaData()
{
...
QString FileNode::GetFileType (const QString& in_str) // standardize the string
{
...
QString FileNode::standardizeType (const QString in)
{
    LIBMTP_filetype_t thistype = stringToType(in.toUpper());
    QString standardType = typeToString(thistype);
    return standardType;
}

QString FileNode::typeToString (LIBMTP_filetype_t in_type)
{
    switch (in_type)
    {
...
LIBMTP_filetype_t FileNode::stringToType()
{
...
LIBMTP_filetype_t FileNode::stringToType(const QString& in_type)
{
    if (in_type == "UNKNOWN")
        return  LIBMTP_FILETYPE_UNKNOWN;
